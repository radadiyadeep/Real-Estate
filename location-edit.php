          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <div class="card-body">
                  ...
                  <div class="row">
                  <input type="hidden" class="form-control" name="location_id"  value="<?php echo $row_list['location_id']; ?>"  required>
                    
                  <div class="col-sm-3">
                     
                      <div class="form-group">
                        <label>Location Name</label>
                        <input type="text" name="location_name"   value="<?php echo $row_list['location_name']; ?>" class="form-control" placeholder="Enter Location Name" required="">
                      </div>
                    </div>
...
if (isset($_POST['update'])) {
  ...
  $id = mysqli_real_escape_string($connection, $_POST['location_id']);
  $location_name = mysqli_real_escape_string($connection, $_POST['location_name']);
   ...
   $query_check = mysqli_query($connection, "select lower(location_name) from $table where location_name=lower('{$location_name}')  and NOT $primary_key = '{$id}'") or die(mysqli_error($connection));
...
{ $msg = "Location Name Already Exist";
  ...
  $queryupdate = mysqli_query($connection, "update $table set `location_name`='{$location_name}' where $primary_key='{$id}'") or die(mysqli_error($connection));
